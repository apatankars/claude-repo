...
with torch.no_grad():
    with ctx:
        for k in range(num_samples):
            ...
            generations_naive.extend(y.tolist())
torch.cuda.synchronize()
t1n = time.time()
print("\nGenerated tokens for first batch:")
print(decode(generations_naive[0]))
print("-----------------------------------------------")
...
with torch.no_grad():
    with ctx:
        for k in range(num_samples):
            ...
            generations_kv.extend(y.tolist())
torch.cuda.synchronize()
t1k = time.time()
print("\nGenerated tokens for first batch:")
print(decode(generations_kv[0]))
print("-----------------------------------------------")
    ...
    with torch.no_grad():
        with ctx:
            for k in range(num_samples):
                ...
                generations_spec.extend(y.tolist())
    torch.cuda.synchronize()
    t1s = time.time()
    print("\nGenerated tokens for first batch:")
    print(decode(generations_spec[0]))
    print("-----------------------------------------------")
else:
    print('\nSkipping speculative inference because batch_size > 1')

tokens_generated = x.size(0) * num_samples * max_new_tokens
print(f'\n[NAIVE] Generated {tokens_generated} tokens in {t1n-t0n:.3f} seconds for {tokens_generated/(t1n-t0n):.3f} tok/sec.')
print(f'[KV]    Generated {tokens_generated} tokens in {t1k-t0k:.3f} seconds for {tokens_generated/(t1k-t0k):.3f} tok/sec ({(t1n-t0n)/(t1k-t0k):.3f}x speed-up).')
if x.size(0) == 1:
    print(f'[SPEC]  Generated {tokens_generated} tokens in {t1s-t0s:.3f} seconds for {tokens_generated/(t1s-t0s):.3f} tok/sec ({(t1n-t0n)/(t1s-t0s):.3f}x speed-up).')

# verify matched predictions
all_matched = True
num_matching = len([1 for i in range(len(generations_naive)) if generations_naive[i] == generations_kv[i]])
print(f'NAIVE and KV generations matched on {num_matching}/{len(generations_naive)} positions')
...
if x.size(0) == 1:
    ...
    print(f'NAIVE and SPEC generations matched on {num_matching}/{len(generations_naive)} positions')
    if num_matching < len(generations_naive):
        all_matched = False
...
if all_matched:
    ...
    print("NVIDIA's underlying libaries aren't wholly deterministic, so don't worry about getting an exact match as long as you're getting good outputs")
    print("You can set `DEBUG` to `True` in `model.py` to make operations much more deterministic, at the cost of speed")